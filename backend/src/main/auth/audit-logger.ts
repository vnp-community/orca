  ...
  async log(entry: AuditEntry): Promise<void> {
    ...
    await this.pool.withConnection((db) =>
      db.query(
        ...
        // call silently failed inside the .catch() below. `userAgent` has no
        // column in the real schema — dropped from the INSERT (AuditEntry.userAgent
        // stays optional on the type, just no longer persisted).
