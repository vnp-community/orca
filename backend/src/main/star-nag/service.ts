...
export class StarNagService {
  ...
  private broadcastShow(
    ...
    // Why: server/backend mode has no local renderer — star-nag's audience
    // here is starNag.subscribe RPC callers (remote/mobile), not a
    // BrowserWindow. Desktop bails out when getAllWindows() is empty because
    // that means no renderer is around to show the card to; in server mode
    // getAllWindows() is *always* empty (no window is ever created for the
    // RPC/WebSocket transport), so keeping that gate would silently disable
    // the whole star-nag feature instead of degrading gracefully — mirrors
    // RuntimeConnectionSubscriptionNotifyCommands.dispatchMobileNotification,
    // which never gates its fan-out on window presence either.
