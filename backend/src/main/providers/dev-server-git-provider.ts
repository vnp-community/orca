...
// FIX BUG-BE-HLD-018 (TASK-HLD-028/029/030): the last NOT_SUPPORTED stub in
// this file (getStagedCommitContext) was replaced with a real implementation
// — all 9 previously-unsupported IGitProvider methods are now wired to relay
// calls, so this helper has no remaining callers.
...
export class DevServerGitProvider implements IGitProvider {
  ...
  async getStagedCommitContext(worktreePath: string): Promise<CommitMessageDraftContext | null> {
    // FIX BUG-BE-HLD-018: diff --cached / branch --show-current are already
    // in ALLOWED_GIT_SUBCOMMANDS, so this composes over the existing this.exec()
    // relay call — mirrors SshGitProvider.getStagedCommitContext.
