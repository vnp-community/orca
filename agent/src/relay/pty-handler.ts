...
export class PtyHandler {
  ...
  private async spawn(
    ...
    const spawnEnv = this.buildSpawnEnv(
      ...
      // `string | undefined`) but only ever assign string literals — safe to
      // narrow back to Record<string, string> for buildSpawnEnv's signature.
      providerEnv ? ({ ...env, ...providerEnv } as Record<string, string>) : env,
