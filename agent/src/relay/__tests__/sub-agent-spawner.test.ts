...
describe('buildAgentEnv', () => {
  ...
  it('HOME và PATH are set', async () => {
    // BUG-AG-HLD-002: buildAgentEnv() now throws when accountId is set, a
    // credential-store lookup applies, and no resolvedApiKey was provided
    // (it never falls back to injecting Layer-1 ciphertext). This test only
    // cares about the base env fields, so omit accountId to skip that branch.
    const env = await buildAgentEnv({ ...baseReq, accountId: '' }, claudeSpec, MOCK_CONFIG_ENV, null)
