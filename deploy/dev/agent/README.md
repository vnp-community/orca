...
> **Source code:** `src/relay/agent-entry.ts` (TypeScript)  
> **Build output:** `out/relay/agent.js` (bundled, self-contained)  
> **Deploy:** `scp out/relay/agent.js ubuntu@<devserver>:~/orca-agent/agent.js`
...
13-byte binary header (phải khớp với `src/main/ssh/relay-protocol.ts`):
```
...
bash deploy/dev/scripts/connect-agent.sh --deploy --mode=relay-ws
```
...
# → out/relay/agent.js (self-contained, ~185KB)

# 2. Deploy lên dev server
scp out/relay/agent.js ubuntu@172.20.2.31:~/orca-agent/agent.js
ssh ubuntu@172.20.2.31 "cd ~/orca-agent && cp .env.example .env"
# Điền ORCA_URL, AGENT_TOKEN vào .env

# 3. Lấy agent token (direct-ws mode)
bash deploy/dev/scripts/connect-agent.sh  # prints token
...
```bash
# Rebuild sau khi sửa src/relay/agent-*.ts
pnpm run build:agent
# → out/relay/agent.js + out/relay/.agent-version
