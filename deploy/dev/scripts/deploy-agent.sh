...
if [ -z "$AGENT_JS_LOCAL" ]; then
  ...
  AGENT_JS_LOCAL="$REPO_ROOT/out/relay/agent.js"
  [ ! -f "$AGENT_JS_LOCAL" ] && AGENT_JS_LOCAL="$REPO_ROOT/deploy/dev/agent/agent.js"
fi
...
# Regenerate: ./deploy/dev/scripts/deploy-agent.sh $TARGET_HOST $DEV_SERVER_ID
exec >> "$LOG_FILE" 2>&1
set -uo pipefail
TS() { date -u +%FT%TZ; }
