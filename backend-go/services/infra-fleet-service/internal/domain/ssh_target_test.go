...
func TestNewSshTarget_ValidatesInvariants(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			target, err := NewSshTarget("st1", tt.tenantID, tt.host, 0, tt.userName, tt.vaultSSHRole, "", "")
			if tt.wantErr == nil {
				if err != nil {
...
func TestNewSshTarget_NeverStoresRawKeyMaterial(t *testing.T) {
	...
	target, err := NewSshTarget("st1", "t1", "host", 0, "user", "role", "", "")
	if err != nil {
...
func TestNewSshTarget_PortKnownHostsJumpHostRoundTrip(t *testing.T) {
	target, err := NewSshTarget("st1", "t1", "host", 2222, "user", "role", "SHA256:abc", "bastion-1")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if target.Port != 2222 {
