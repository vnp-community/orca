...
// mid-backoff. direct-websocket/relay-ssh/ephemeral-VM sessions carry a
// non-default managedMode — see session.go's doc comment on
// backgroundReconnect for why they don't auto-reconnect the same way.
type Client struct {
	cfg    Config
	logger *slog.Logger
