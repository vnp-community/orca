...
func TestClientExecStream_DeliversFramesInOrderThenCloses(t *testing.T) {
	...
	devServer, err := domain.NewDevServer("ds-execstream", "tenant-1", host, domain.ConnectionModeRelayWebSocket, "")
	if err != nil {
		t.Fatalf("NewDevServer: %v", err)
	}
...
func TestClientExecStream_RelaySSHModeErrors(t *testing.T) {
	...
	devServer, err := domain.NewDevServer("ds-ssh", "tenant-1", "unused", domain.ConnectionModeRelaySSH, "target-1")
	if err != nil {
		t.Fatalf("NewDevServer: %v", err)
	}
