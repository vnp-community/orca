...
// the returned diagnosticStderr is live and capped, per TASK-SSH-02-06.
func TestLaunch_ReturnsACappedStderrBuffer(t *testing.T) {
	...
	transport, stderrBuf, err := sshrelay.LaunchForTest(conn, sshrelay.RemoteDirForTest, "ds-launch-1")
	if err != nil {
	...
	// server's --stdio handler never itself writes to the SSH channel's
