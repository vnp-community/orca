...
func TestConnectAndRunCommand_SucceedsAgainstFakeServer(t *testing.T) {
	...
	target, err := domain.NewSshTarget("target-1", "tenant-1", "127.0.0.1", server.port(t), "deploy", "role-1", "", "")
	if err != nil {
...
func TestConnect_FailsWhenIssuerErrors(t *testing.T) {
	...
	target, err := domain.NewSshTarget("target-2", "tenant-1", "127.0.0.1", server.port(t), "deploy", "role-1", "", "")
	if err != nil {
		t.Fatalf("NewSshTarget: %v", err)
	}
...
func TestConnect_FailsWhenServerRejectsCert(t *testing.T) {
	...
	target, err := domain.NewSshTarget("target-3", "tenant-1", "127.0.0.1", server.port(t), "deploy", "role-1", "", "")
	if err != nil {
		t.Fatalf("NewSshTarget: %v", err)
	}
...
func TestConnect_DialsThroughJumpHost(t *testing.T) {
	...
	bastionTarget, err := domain.NewSshTarget("bastion-1", "tenant-1", "127.0.0.1", bastion.port(t), "deploy", "role-1", "", "")
	if err != nil {
		t.Fatalf("NewSshTarget(bastion): %v", err)
	}
	finalTarget, err := domain.NewSshTarget("target-jump", "tenant-1", "127.0.0.1", realTarget.port(t), "deploy", "role-1", "", "bastion-1")
	if err != nil {
...
func TestConnect_KnownHostsMismatchRejectsDial(t *testing.T) {
	...
	target, err := domain.NewSshTarget("target-mismatch", "tenant-1", "127.0.0.1", server.port(t), "deploy", "role-1", "SHA256:not-the-real-fingerprint", "")
	if err != nil {
		t.Fatalf("NewSshTarget: %v", err)
	}
...
func TestConnect_UnreachableHostReturnsTypedError(t *testing.T) {
	...
	target, err := domain.NewSshTarget("target-unreachable", "tenant-1", "127.0.0.1", port, "deploy", "role-1", "", "")
	if err != nil {
		t.Fatalf("NewSshTarget: %v", err)
	}
...
func TestKeepAlive_SendsRequestsAtInterval(t *testing.T) {
	...
	target, err := domain.NewSshTarget("target-keepalive", "tenant-1", "127.0.0.1", server.port(t), "deploy", "role-1", "", "")
	if err != nil {
		t.Fatalf("NewSshTarget: %v", err)
	}
	issuer := &fakeIssuer{ca: ca, principal: "deploy"}
