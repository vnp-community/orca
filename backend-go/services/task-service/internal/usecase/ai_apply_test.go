...
// TASK-TG-002-02's core new-behavior test: a proposal's DependsOnIndices
...
func TestAIApply_DependsOnIndex_CreatesDependsOnEdgesAmongSiblings(t *testing.T) {
	...
	// AddEdge's "from depends_on to" convention: FromTaskID is the
	// DEPENDENT (waits), ToTaskID is the dependency (must finish first) —
	// see add_edge.go's auto-block doc comment.
...
func TestAIApply_DependsOnIndex_SameBatchCycle_RollsBackWholeBatch(t *testing.T) {
	...
	proposals := []domain.SubtaskProposal{
		{Title: "A", DependsOnIndices: []int{1}},
		{Title: "B", DependsOnIndices: []int{0}},
	}
	created, err := uc.Execute(ctx, AIApplyInput{TaskID: "parent", Proposals: proposals})
	if err == nil {
