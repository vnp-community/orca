...
func (uc *GenerateShareLink) Execute(ctx context.Context, in GenerateShareLinkInput) (string, error) {
	...
	// events is nil — minting a share token has no outbox-published fact of
	// its own (see TaskRepository.Update's doc comment: events may be empty).
	if err := uc.tasks.Update(ctx, tenantID, task, nil); err != nil {
