...
func (f *fakeProjectInfoResolver) Resolve(ctx context.Context, tenantID, projectID string) (string, string, error) {
...
func newAIDecomposeForTest(tasks TaskRepository, edges EdgeRepository, aiProvider AIProviderContextResolver, resolver ProjectExecutionResolver, relay AICompleter) *AIDecompose {
	return NewAIDecompose(tasks, edges, aiProvider, resolver, &fakeProjectInfoResolver{}, &fakeTechStackDetector{}, &fakeVelocityResolver{}, relay)
}
...
func TestAIDecompose_TechStackDetectorFailure_DoesNotFailExecute(t *testing.T) {
	...
	uc := NewAIDecompose(
		...
		&fakeProjectInfoResolver{}, &fakeTechStackDetector{err: errors.New("detect failed")},
...
func TestAIDecompose_PromptIncludesEveryContextSource(t *testing.T) {
	...
	uc := NewAIDecompose(
		...
		&fakeProjectExecutionResolver{connectionID: "conn-1", connected: true},
		&fakeProjectInfoResolver{name: "MyProject", repoURL: "https://example.com/repo.git"},
