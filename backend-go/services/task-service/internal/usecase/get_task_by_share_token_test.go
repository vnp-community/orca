...
func TestGetTaskByShareToken_ReturnsExactlyTheAllowlistedFields(t *testing.T) {
	...
	sensitiveAIContext := json.RawMessage(`"internal repo secrets and architecture notes"`)
	sensitiveAIPlan := json.RawMessage(`{"plan":"internal decomposition strategy"}`)
	task := domain.Task{
		ID:             "t1",
		TenantID:       "tenant-1",
		Title:          "Public title",
		...
		ParentID:       "parent-1",
		ProjectID:      "proj-1",
		AssigneeID:     strPtr("assignee-1"),
		ReporterID:     strPtr("reporter-1"),
		OwnerID:        strPtr("owner-1"),
		PromptTemplate: "secret prompt template",
		AIContext:      sensitiveAIContext,
		AIPlanJSON:     sensitiveAIPlan,
		Visibility:     "private",
		WorktreeID:     strPtr("worktree-1"),
		AgentSessionID: strPtr("session-1"),
		WorkflowExecID: strPtr("exec-1"),
		ShareToken:     strPtr("the-real-token"),
	}
	tasks.tasks["t1"] = task
