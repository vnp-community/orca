...
func TestRepository_GetSubtree_ReturnsOnlyDescendants(t *testing.T) {
	...
	rootSubtree, _, err := repo.GetSubtree(ctx, tenantID, root.ID, 0)
	if err != nil {
	...
	aSubtree, _, err := repo.GetSubtree(ctx, tenantID, a.ID, 0)
	if err != nil {
	...
	// wrong tenant) must find nothing — the explicit tenant_id filter in the
	// anchor term means zero rows come back, which GetSubtree surfaces as a
	// not-found error (see its own doc comment), never otherRoot's real
	// subtree leaking through.
	if _, _, err := repo.GetSubtree(ctx, tenantID, otherRoot.ID, 0); err == nil {
		t.Error("expected an error for a task ID under the wrong tenant, got none")
	}
}
