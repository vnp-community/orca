...
func (b *Bootstrap) EnsureAdmin(ctx context.Context, cfg BootstrapConfig, logger *slog.Logger) (generatedPassword string, err error) {
	...
	if entry, err := domain.NewAuditEntry(uuid.NewString(), created.TenantID, created.ID, "user.bootstrap_created", "", "user", created.ID, map[string]any{}, domain.OutcomeAllowed, "", now); err == nil {
