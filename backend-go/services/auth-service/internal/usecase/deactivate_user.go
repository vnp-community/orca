...
func (uc *DeactivateUser) Execute(ctx context.Context, userID string) (domain.User, error) {
	...
	now := uc.clock.Now()
	if entry, err := domain.NewAuditEntry(uuid.NewString(), updated.TenantID, actor.ID, "user.deactivated", "", "user", updated.ID, map[string]any{}, domain.OutcomeAllowed, "", now); err == nil {
