...
func (uc *RevokeSession) Execute(ctx context.Context, sessionToken string) error {
	...
	if entry, err := domain.NewAuditEntry(uuid.NewString(), session.TenantID, actor.ID, "session.revoked", "", "session", tokenHash, map[string]any{}, domain.OutcomeAllowed, "", now); err == nil {
