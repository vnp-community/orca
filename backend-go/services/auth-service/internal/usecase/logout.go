...
func (uc *Logout) Execute(ctx context.Context, sessionToken string) error {
	...
	if entry, err := domain.NewAuditEntry(uuid.NewString(), session.TenantID, session.UserID, "user.logout", "", "session", tokenHash, map[string]any{}, domain.OutcomeAllowed, "", now); err == nil {
