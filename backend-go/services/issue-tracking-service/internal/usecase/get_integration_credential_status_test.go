...
type fakeCredentialStatusReader struct {
	getStatusFunc func(ctx context.Context, tenantID string, provider domain.Provider) (bool, string, error)
}

func (f *fakeCredentialStatusReader) GetStatus(ctx context.Context, tenantID string, provider domain.Provider) (bool, string, error) {
	return f.getStatusFunc(ctx, tenantID, provider)
}

func TestGetIntegrationCredentialStatus_NotConfigured(t *testing.T) {
	reader := &fakeCredentialStatusReader{getStatusFunc: func(context.Context, string, domain.Provider) (bool, string, error) {
		return false, "", nil
	}}
	uc := usecase.NewGetIntegrationCredentialStatus(reader)

	got, err := uc.Execute(context.Background(), usecase.GetIntegrationCredentialStatusInput{TenantID: "t1", Provider: domain.ProviderLinear})
	if err != nil {
...
func TestGetIntegrationCredentialStatus_Configured(t *testing.T) {
	...
	got, err := uc.Execute(context.Background(), usecase.GetIntegrationCredentialStatusInput{TenantID: "t1", Provider: domain.ProviderJira})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if !got.Configured {
