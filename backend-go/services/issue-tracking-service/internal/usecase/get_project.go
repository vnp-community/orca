...
func NewGetProject(registry ProviderRegistry, credentials CredentialResolver) *GetProject {
	return &GetProject{registry: registry, credentials: credentials}
}

func (uc *GetProject) Execute(ctx context.Context, in GetProjectInput) (domain.ProjectRef, error) {
	...
	project, err := provider.GetProject(ctx, cred, in.ProjectID, in.WorkspaceID)
	if err != nil {
		return domain.ProjectRef{}, apperrors.New(apperrors.KindInternal, "ISSUETRACKING_GET_PROJECT_FAILED", "failed to get project", err)
	}
	return project, nil
}
