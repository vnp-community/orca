...
// fail this service's startup). Shared by every real client constructor
// that needs to dial a downstream service (infrafleetclient.WorkerDispatcher,
// taskserviceclient.Reporter) so cmd/server/main.go can reuse one
// connection per downstream service.
//
