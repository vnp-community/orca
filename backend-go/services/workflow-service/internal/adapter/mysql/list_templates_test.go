...
func mustCreateTemplate(t *testing.T, repo *Repository, tenantID, name string, opts ...domain.TemplateOption) domain.WorkflowTemplate {
	...
	// rating_count (mirrors internal/adapter/postgres's identical
	// CreateTemplate — those only ever move via IncrementUsageCount/
	// UpsertRating in the real system); tests that need specific seed
	// values for the trending sort write them directly.
	if tmpl.UsageCount != 0 || tmpl.RatingSum != 0 || tmpl.RatingCount != 0 {
		_, err := repo.db.ExecContext(context.Background(),
			`UPDATE templates SET usage_count = ?, rating_sum = ?, rating_count = ? WHERE id = ?`,
...
// TestListTemplates_QueryFullTextMatchesNameOrDescription exercises the
// FULLTEXT index (idx_workflow_templates_fts) this dialect uses in place
// of Postgres's tsvector/GIN index — see
// migrations/mysql/0009_template_visibility_sharing.up.sql's comment on
// the recall-behavior difference this doesn't paper over.
...
// TestListTemplates_TagsFilterRequiresAllListedTags exercises
// tagConditions' JSON_CONTAINS-per-tag AND-filter — the MySQL/TiDB
// equivalent of Postgres's `tags @> $N::text[]` containment operator (see
// repository.go's tagConditions doc comment).
