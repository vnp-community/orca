...
func (r *ProjectContextResolver) GetProjectContext(ctx context.Context, projectID string) (usecase.ProjectContext, error) {
	...
	// which only covers the infra-fleet-service Relay calls this package's
	// other files make). The caller (AgentExecutor) is expected to have
	// already set tenant.WithUserID(ctx, cfg.UserID) for the step's target
	// user before reaching this resolver — see TASK-PRF-04-06.
	tenantID, err := tenant.RequireTenantID(ctx)
	if err != nil {
		return usecase.ProjectContext{}, fmt.Errorf("infrafleetclient: project context resolver: %w", err)
	}
