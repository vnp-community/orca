...
func (uc *StashPop) Execute(ctx context.Context, in StashPopInput) (domain.MergeResult, error) {
	if in.WorktreeID == "" {
		return domain.MergeResult{}, apperrors.New(apperrors.KindInvalidArgument, "GITGATEWAY_MISSING_WORKTREE_ID", "worktree_id is required", nil)
	}
	...
	if conn.Connected && conn.Mode == infrafleetv1.ConnectionMode_CONNECTION_MODE_RELAY_SSH {
		return domain.MergeResult{}, apperrors.New(apperrors.KindFailedPrecondition, "GITGATEWAY_STASH_POP_UNSUPPORTED_SSH_RELAY", "stash pop is not supported over an SSH-relay connection", domain.ErrGitOpUnsupportedOverSSHRelay)
	}
	executor := uc.local
	if conn.Connected {
		executor = uc.relay
	...
	if err != nil {
		return domain.MergeResult{}, apperrors.New(apperrors.KindInternal, "GITGATEWAY_STASH_POP_FAILED", "failed to pop stash", err)
	}
	return result, nil
}
