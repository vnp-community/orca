...
func (uc *MergeBranch) Execute(ctx context.Context, in MergeBranchInput) (domain.MergeOutcome, error) {
	if in.WorktreeID == "" || in.Branch == "" {
		return domain.MergeOutcome{}, apperrors.New(apperrors.KindInvalidArgument, "GITGATEWAY_MISSING_ARGS", "worktree_id and branch are required", nil)
	}
	...
	if conn.Connected && conn.Mode == infrafleetv1.ConnectionMode_CONNECTION_MODE_RELAY_SSH {
		return domain.MergeOutcome{}, apperrors.New(apperrors.KindFailedPrecondition, "GITGATEWAY_MERGE_UNSUPPORTED_SSH_RELAY", "merge is not supported over an SSH-relay connection", domain.ErrGitOpUnsupportedOverSSHRelay)
	}
	executor := uc.local
	if conn.Connected {
		executor = uc.relay
	}
	result, err := executor.MergeIntoBranch(ctx, conn.RepoPath, in.Branch, in.NoFF)
	if err != nil {
		return domain.MergeOutcome{}, apperrors.New(apperrors.KindInternal, "GITGATEWAY_MERGE_FAILED", "failed to merge branch", err)
	}
	return result, nil
}
