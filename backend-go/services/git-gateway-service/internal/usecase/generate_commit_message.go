...
func NewGenerateCommitMessage(resolver ConnectionResolver, getDiff *GetDiff, completer AICompleter) *GenerateCommitMessage {
	return &GenerateCommitMessage{resolver: resolver, getDiff: getDiff, completer: completer}
}
...
func (uc *GenerateCommitMessage) Execute(ctx context.Context, in GenerateCommitMessageInput) (string, error) {
	...
	// Reuse GetDiff's own resolve-and-dispatch rather than calling a
	// GitExecutor directly here, so diff-fetching logic (and its error
	// wrapping) stays owned in exactly one place. This does mean
	// ResolveConnection is called twice per request (once above to decide
	// the AI-relay path, once again inside GetDiff.Execute) — acceptable
	// duplication for a stateless lookup, traded for not re-deriving
	// dispatchExecutor's routing here.
	diff, err := uc.getDiff.Execute(ctx, GetDiffInput{WorktreeID: in.WorktreeID, Staged: true})
	if err != nil {
		return "", err
	}

	message, err := uc.completer.Complete(ctx, conn.ConnectionID, commitMessagePromptPrefix+diff.UnifiedDiff)
	if err != nil {
