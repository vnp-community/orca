...
// IssueTrackingService, always with Provider: ISSUE_PROVIDER_JIRA. View
// types below mirror frontend/src/shared/jira-types.ts field-for-field —
// check that file before changing a field name here.
package wscompat
...
type jiraCreateFieldView struct {
	Key               string `json:"key"`
	Name              string `json:"name"`
	...
	AllowedValuesJSON string `json:"allowedValuesJson,omitempty"`
}

func toJiraCreateFieldView(f *issuetrackingv1.CreateField) jiraCreateFieldView {
	return jiraCreateFieldView{
		Key: f.GetKey(), Name: f.GetName(), Required: f.GetRequired(),
		SchemaType: f.GetSchemaType(), SchemaItems: f.GetSchemaItems(), SchemaCustom: f.GetSchemaCustom(),
		AllowedValuesJSON: f.GetAllowedValuesJson(),
	}
}
...
type jiraTransitionView struct {
	ID   string         `json:"id"`
	Name string         `json:"name"`
	To   jiraStatusView `json:"to"`
}

func toJiraTransitionView(t *issuetrackingv1.Transition) jiraTransitionView {
	v := jiraTransitionView{ID: t.GetId(), Name: t.GetName()}
	if to := t.GetTo(); to != nil {
		v.To = jiraStatusView{ID: to.GetId(), Name: to.GetName()}
	}
	return v
}

func attachIdentity(ctx context.Context, id Identity) context.Context {
...
func registerJiraChannels(r *Registry, client issuetrackingv1.IssueTrackingServiceClient) {
	...
	r.Register("jira.listCreateFields", func(ctx context.Context, id Identity, args []json.RawMessage) (any, error) {
		...
		views := make([]jiraCreateFieldView, 0, len(resp.GetFields()))
		for _, f := range resp.GetFields() {
			views = append(views, toJiraCreateFieldView(f))
		}
		return views, nil
	})
	...
	r.Register("jira.listTransitions", func(ctx context.Context, id Identity, args []json.RawMessage) (any, error) {
		...
		views := make([]jiraTransitionView, 0, len(resp.GetTransitions()))
		for _, t := range resp.GetTransitions() {
			views = append(views, toJiraTransitionView(t))
		}
		return views, nil
	})
