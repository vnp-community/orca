...
// rpcTimeout is the standard per-call deadline every handler in this file
// (other than the two explicitly longer-deadline calls below) uses for its
// outbound gRPC call — mirrors the same constant name/value channels.go's
// other register*Channels functions assume already exists in this package.
// If another parallel group's channels.go changes also define `rpcTimeout`,
// the integration pass must dedupe to a single declaration.
