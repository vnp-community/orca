...
func registerCliInstallerChannels(r *Registry, client infrafleetv1.InfraFleetServiceClient) {
	r.Register("cli.getInstallStatus", cliInstallerStatusHandler(client, "cli.getInstallStatus"))
	r.Register("cli.install", cliInstallerMutationHandler(client, "cli.install"))
	r.Register("cli.remove", cliInstallerMutationHandler(client, "cli.remove"))
	r.Register("cli.getWslInstallStatus", cliInstallerWslStatusHandler(client, "cli.getWslInstallStatus"))
	r.Register("cli.installWsl", cliInstallerWslMutationHandler(client, "cli.installWsl"))
	r.Register("cli.removeWsl", cliInstallerWslMutationHandler(client, "cli.removeWsl"))
}

// cliInstallerStatusHandler backs cli.getInstallStatus — a read, so a dev server with
// no live agent session degrades to an honest "unsupported" CliInstallStatus
// (cliInstallerNotConnectedStatus) instead of erroring, same tolerance
// onboardingDetectAgents already applies to preflight.detectAgents.
func cliInstallerStatusHandler(client infrafleetv1.InfraFleetServiceClient, agentMethod string) ChannelHandler {
	return func(ctx context.Context, id Identity, args []json.RawMessage) (any, error) {
		...
		result, err := relayCliInstallerMethod(ctx, id, client, in.DevServerID, agentMethod, "{}")
		if err != nil {
			if status.Code(err) == codes.FailedPrecondition {
				return cliInstallerNotConnectedStatus("Dev server is not connected."), nil
			}
			return nil, err
		}
		return result, nil
...
// cliInstallerMutationHandler backs cli.install/cli.remove — unlike the status
...
func cliInstallerMutationHandler(client infrafleetv1.InfraFleetServiceClient, agentMethod string) ChannelHandler {
	return func(ctx context.Context, id Identity, args []json.RawMessage) (any, error) {
		in, err := decodeArg[cliDevServerArgs](args, 0)
		if err != nil {
			return nil, err
		...
		return relayCliInstallerMethod(ctx, id, client, in.DevServerID, agentMethod, "{}")
	}
}

func cliInstallerWslStatusHandler(client infrafleetv1.InfraFleetServiceClient, agentMethod string) ChannelHandler {
	return func(ctx context.Context, id Identity, args []json.RawMessage) (any, error) {
		...
		result, err := relayCliInstallerMethod(ctx, id, client, in.DevServerID, agentMethod, string(paramsJSON))
		if err != nil {
			if status.Code(err) == codes.FailedPrecondition {
				return cliInstallerNotConnectedStatus("Dev server is not connected."), nil
			}
...
func cliInstallerWslMutationHandler(client infrafleetv1.InfraFleetServiceClient, agentMethod string) ChannelHandler {
	return func(ctx context.Context, id Identity, args []json.RawMessage) (any, error) {
		in, err := decodeArg[cliWslArgs](args, 0)
		if err != nil {
			return nil, err
		...
		return relayCliInstallerMethod(ctx, id, client, in.DevServerID, agentMethod, string(paramsJSON))
	}
}

// relayCliInstallerMethod is this file's one RelayByDevServer call site — every
...
// cliInstallerNotConnectedStatus is the CliInstallStatus-shaped (frontend/src/shared/
...
func cliInstallerNotConnectedStatus(detail string) map[string]any {
