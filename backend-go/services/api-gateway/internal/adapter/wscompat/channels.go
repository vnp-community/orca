...
func RegisterRealChannels(
	...
	// registerCliInstallerChannels: called here on origin/main before this
	// merge, but never defined anywhere in either merge parent's history —
	// a pre-existing gap on origin/main, not something this merge removed.
	// Needs its own implementation (and a decision on what "CLI installer"
	// channels this was meant to expose) before being wired back in.
	...
	registerSCMChannels(r, scmClient, gitClient, annotationClient)
