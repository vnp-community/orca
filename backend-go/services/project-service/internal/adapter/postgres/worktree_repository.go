...
const worktreeColumns = `id, project_id, repo_id, path, branch, active, COALESCE(linked_issue_provider, ''), COALESCE(linked_issue_ref, '')`
...
func scanWorktree(row rowScanner) (domain.Worktree, error) {
	...
	if err := row.Scan(&wt.ID, &wt.ProjectID, &wt.RepoID, &wt.Path, &wt.Branch, &wt.Active, &wt.LinkedIssueProvider, &wt.LinkedIssueRef); err != nil {
