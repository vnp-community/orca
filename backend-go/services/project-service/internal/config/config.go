...
type Config struct {
	...
	// NATSURL is the transactional-outbox relay's NATS connection (SOL-WT-01)
	// — RecordWorktreeCreated's worktree.created event, same
	// enqueue-in-transaction/poll-and-publish pattern usage-service already
	// ships.
	NATSURL string
}
