...
type HandleIncomingEvent struct {
	...
	// notifications persists the unread-state row (CR-NOTIF-001,
	// TASK-BE-NOTIF-003) — nil-safe, same convention as deliverPush, so a
	// test that only cares about WS fan-out doesn't need to wire it.
...
func NewHandleIncomingEvent(broadcaster NotificationBroadcaster, processedEvents ProcessedEventRepository, notifications NotificationRepository, deliverPush *DeliverPush, logger *slog.Logger) *HandleIncomingEvent {
	if logger == nil {
		logger = slog.Default()
	}
	return &HandleIncomingEvent{broadcaster: broadcaster, processedEvents: processedEvents, notifications: notifications, deliverPush: deliverPush, logger: logger}
}
...
func (uc *HandleIncomingEvent) Execute(ctx context.Context, in HandleIncomingEventInput) error {
	...
	// Persist BEFORE Broadcast (TASK-BE-NOTIF-003) — a crash between the
	// two leaves a persisted-but-not-yet-broadcast record (recoverable via
	// ListNotifications on next load), never a broadcast-but-lost one. A
	// persistence failure is logged, not fatal — the WS/push paths below
	// still complete, matching this usecase's "a downstream hiccup must
	// never NAK the whole event" posture.
	if uc.notifications != nil {
		if err := uc.notifications.SaveNotificationEvent(ctx, event); err != nil {
			uc.logger.WarnContext(ctx, "failed to persist notification event",
