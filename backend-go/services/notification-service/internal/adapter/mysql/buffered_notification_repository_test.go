...
// Mirrors internal/adapter/postgres/buffered_notification_repository_test.go
...
// undelivered row, so the count never exceeds 50 — also exercises this
// adapter's Go-generated id (migrations/mysql/0003's comment: no DB-side
// default here, unlike the Postgres table).
func TestBufferedNotificationStore_Enqueue_CapsAt50PerSubscription(t *testing.T) {
	repo := setupRepository(t)
	store := NewBufferedNotificationStore(repo.db)
