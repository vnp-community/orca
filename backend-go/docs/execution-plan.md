...
- [ ] **`relay-ssh` mode still not implemented — but its SSH connection
      layer now is** (2026-08-17, third pass) — `internal/adapter/sshconn` opens
      real, Vault-cert-authenticated SSH connections and runs commands, but
      is deliberately NOT wired into `devserveragent.Client`/any RPC: the
      mode's actual point (SFTP-deploy + launch `relay.js`, then talk
      JSON-RPC over the exec channel) needs a `relay.js` build artifact this
      repo's build has no path to at all (it's `agent/`'s Electron-packaged
      output, not something backend-go produces or can reach). `Exec`/`Health`
      for relay-ssh-mode dev servers still return
      `ErrConnectionModeNotImplemented`, honestly — see below for the full
      reasoning.
