...
For where each of these methods actually executes once it reaches the
backend — processed in-process against PostgreSQL, or relayed to the Dev
Server Agent — see
[`backend-agent-execution-boundary.md`](./backend-agent-execution-boundary.md).
This doc only tells you the method exists and who calls it; that one tells
you what happens after the backend receives it.
