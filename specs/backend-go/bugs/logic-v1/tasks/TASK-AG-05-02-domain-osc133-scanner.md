# TASK-AG-05-02: `domain/osc133_scanner.go` — Go port of the chunk-boundary-safe OSC 133 scanner
...
Ports `agent/src/shared/terminal-osc133-command-finished.ts`'s carry-buffer algorithm to Go — same split-chunk-safe scan, same two markers. **Correction to SOL-AG-05's sketch**: the real TS file only handles OSC 133;C ("command started") and OSC 133;D ("command finished", with a best-effort exit code) — there is no `A` (prompt-start) marker in this scanner; the SOL's "A=start, C=exec, D=finished" table over-stated the marker set. This port matches the real two-marker algorithm exactly.

## Changes to make

Create `backend-go/services/infra-fleet-service/internal/domain/osc133_scanner.go`:
...
go test ./services/infra-fleet-service/internal/domain/... -run TestOsc133Scanner -v
```

Add `osc133_scanner_test.go`, porting the exact split-chunk test cases from
`agent/src/shared/__tests__/terminal-osc133-command-finished.test.ts`
(single-chunk BEL-terminated, single-chunk ST-terminated, prefix split
across two `Feed` calls, terminator split across two `Feed` calls, exit
code present/absent/malformed) so the Go port's behavior is verified
against the same fixtures the TS original uses, not independently invented
ones.
