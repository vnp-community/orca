...
**Status:** `[x]` DONE — `generate_share_link.go`/`preview_shared_template.go`/`import_shared_template.go` implement the token-mint/preview/cross-tenant-import trio; `ResolveTemplate` gained a `Resolve(ctx, tenantID, templateID)` core (Execute stays ctx-tenant-scoped, calls it) so `ImportSharedTemplate` can resolve the SOURCE template's own tenant's chain rather than the importer's. Wired into `cmd/server/main.go` + `grpc/server.go` (3 new handlers). Fixed a shape mismatch while wiring: `SharedTemplatePreview` originally exposed only a derived `AverageRating`, but the proto message needs raw `rating_sum`/`rating_count` — added `RatingSum` alongside `AverageRating` rather than lossily back-deriving an int from a float in the gRPC layer. New tests (11 total): `generate_share_link_test.go` (non-public rejected, token round-trips via `GetByShareToken`, repeated calls return the SAME token — idempotent, unknown template not-found), `preview_shared_template_test.go` (unknown token and a since-unpublished token return the IDENTICAL `WORKFLOW_SHARE_LINK_INVALID` error, valid-token projection populated correctly including `RatingSum`/`RatingCount`/`AverageRating`, and a reflection-based assertion that `SharedTemplatePreview`'s field set structurally excludes anything owner/tenant-named), `import_shared_template_test.go` (cross-tenant import lands under the importer's tenant with empty `ParentTemplateID` + `ClonedFromTemplateID` set + the resolved DAG, since-unpublished fails identically to Preview, unknown token not-found). `go build/vet/test ./...` green for workflow-service; api-gateway still builds.
