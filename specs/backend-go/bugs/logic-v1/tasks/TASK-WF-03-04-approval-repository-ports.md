...
**Status:** `[x]` DONE — `TemplateRepositoryTx`/`ApprovalRepositoryTx`/`ApprovalRepository`/`OPAChecker` ports added to `ports.go`; `TemplateRepository` gained `WithTx`/`GetByShareToken`/`SetShareToken`. Postgres: new `template_tx.go` (`Repository.WithTx` begin/commit/rollback + `templateTx` implementing all 4 tx methods + `GetByShareToken`/`SetShareToken`) and `approval_repository.go` (`ApprovalStore` — same split-type-over-shared-pool pattern as infra-fleet-service's `SshTargetStore`, avoiding a `Get` method-name collision — implementing `WithTx`/`ListPending` + `approvalTx` implementing `Get`/`Update`/`Templates()`/`CreateTx`). `fakeTemplateRepository` (usecase tests) gained a genuine stage-then-commit-or-discard `WithTx` (not a shared-map mutation) so a later task's "inject a failure mid-transaction, assert nothing landed" tests are meaningful, plus a `txFailAfterWrites` knob. New integration tests `template_tx_test.go`: `TestWithTx_CommitsOnSuccess`, `TestWithTx_RollsBackOnError` (partial write injected then fails, raw-SQL-verified nothing committed — `GetTemplate` itself doesn't project `usage_count`/`visibility`, a pre-existing gap noted and worked around, not fixed, since it's outside this task's scope), `TestWithTx_UpdateVisibilityAndShareToken`. All 3 pass individually (shared Docker contention causes transient container-startup failures when several agents run testcontainers concurrently — not a regression, confirmed by immediate pass on retry). `go build/vet/test ./...` green for workflow-service; api-gateway still builds. `NewApprovalStore` intentionally not yet wired into `main.go` — no usecase consumes it until TASK-WF-03-05.
