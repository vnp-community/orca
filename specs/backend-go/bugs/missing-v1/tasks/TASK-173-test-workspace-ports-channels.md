...
**Status:** `[x]` DONE (verified) — `kill_workspace_port_test.go` added in `infra-fleet-service/internal/usecase`, extending the existing `fakeConnectionResolver`/`fakeDevServerAgentClient` (added `execCalled`/`lastMethod`/`healthy` tracking fields rather than duplicating). `channels_repo_ssh_status_workspace_test.go` (new file, not `channels_test.go`) covers `workspacePorts.scan`/`workspacePorts.kill` plus a registration-coverage regression guard. `go test` green for both.
