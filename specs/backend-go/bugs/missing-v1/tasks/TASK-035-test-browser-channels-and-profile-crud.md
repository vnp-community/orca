...
**Status:** `[x]` DONE (verified — `go test ./services/infra-fleet-service/internal/usecase/... -run BrowserProfile -v` and `go test ./services/api-gateway/internal/adapter/wscompat/... -run Browser -v` both pass, all tests green. `channels_browser_test.go` also asserts `gotRelay.GetConnectionId() == "conn-1"` against a fake `ResolveConnectionResponse{..., ConnectionId: "conn-1"}` — proving the corrected connectionId (not `DevServer.Id`) is what's threaded through Relay. No live Postgres in this environment — usecase tests use fakes, no DB-integration test was added/run.)
