...
**Status:** `[x]` DONE — `check_hosted_review_eligibility_test.go` implements all 5 named test cases (NotConnected/BranchNotFound/ReviewAlreadyExists/Eligible/PropagatesBranchExistsFailure) plus a RequiresTenantRepoBranch validation test, using `fakeProvider.branchExists`/`branchExistsErr` (added in TASK-082's fake extension) and the exact `fakeCredentialResolverConnectedFlag` this file specifies, matched against `get_auth_status.go`'s real not-connected contract (any `Resolve` error, not a specific sentinel type). `branch_exists_test.go` (200/404) implemented for both github and gitlab adapters. wscompat tests live in `channels_scm_test.go` (TASK-080/081/085's file-location deviation): `TestHostedReviewCreateChannel_Success`, `TestHostedReviewForBranchChannel_ReturnsNilWhenNotFound`, `TestHostedReviewGetCreationEligibilityChannel_Success` all present — Step 4's `TestHostedReviewCreateChannelMatchesRESTContract` was NOT added as a literal cross-package REST-vs-channel HTTP test (`scm_routes.go`'s handlers are unexported in package `httpgateway`, not reachable from package `wscompat`'s tests without exporting them, which was out of scope for this pass); the equivalent regression guard exists as `TestGitLabRateLimitChannelMatchesRESTContract` (same-fake direct-call-vs-channel-dispatch comparison) instead. `go test ./internal/usecase/... ./internal/adapter/github/... ./internal/adapter/gitlab/...` and `go test ./internal/adapter/wscompat/...` both pass; `buf breaking` clean.
