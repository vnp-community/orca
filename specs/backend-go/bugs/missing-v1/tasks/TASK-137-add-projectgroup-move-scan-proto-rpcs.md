...
**Depends on:** none
**Status:** `[x]` DONE (verified) — `ProjectGroup.project_id` field + `MoveProject`/`ScanNested`/`ImportNested` RPCs/messages added to `project.proto`; `buf generate` regenerated stubs cleanly; `go build ./proto/...` green.
