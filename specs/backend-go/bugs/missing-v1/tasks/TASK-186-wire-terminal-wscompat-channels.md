...
>
> ✅ **Correction resolved:** `terminal.multiplex` is now implemented (`channels_terminal_multiplex.go`, new), reusing the exact `AttachPty` relay `terminal.create` already opens — only the OUTPUT ENCODING changed, per this pass's scoping. Read `backend/src/shared/terminal-stream-protocol.ts` in full for the binary frame layout (`terminal_stream_frame.go`'s `EncodeTerminalStreamFrame`/`DecodeTerminalStreamFrame` reproduce it byte-for-byte, locked in by `TestEncodeTerminalStreamFrame_MatchesTSByteLayout`'s hand-computed fixed-byte-sequence assertion) and `backend/src/main/runtime/rpc/methods/terminal.ts`'s `terminal.multiplex` handler (~line 1510) in full for the Subscribe/Input/Resize/Unsubscribe opcode semantics this pass reproduces (ACK flow control, remote-desktop viewport claims, and on-demand snapshots are explicitly out of scope — accepted-but-ignored, never silently broken; see the new file's package doc comment). This required two small, generic transport additions that do NOT alter `Register`/`RegisterStream`/`RegisterStreamChannel`'s existing behavior: `Registry.RegisterBinaryStreamHandler` (`binary_stream_registry.go`) and `Handler.ServeHTTP`'s read loop switching from `wsjson.Read` (which would corrupt-close the connection on any WS BINARY frame — a real bug, now fixed) to a raw `conn.Read` that branches on message type. `terminal.output`/`terminal.exited` remain registered and tested, unmodified — `terminal.multiplex` supplements them. See TASK-187 for the new test list, including an end-to-end test (`TestTerminalMultiplexChannel_EndToEndSubscribeOutputAndInput`) that caught a real lifecycle bug during development (a `BinaryStreamChannelHandler` watching the wrong context tore its session down within microseconds of acking).
