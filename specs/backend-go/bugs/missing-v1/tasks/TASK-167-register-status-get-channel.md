...
**Depends on:** none
**Status:** `[partial]` `registerStatusChannels`/`status.get` implemented in the new `channels_repo_ssh_status_workspace.go` file (not `channels.go`), including `hostPlatformString`/the protocol-version constants. Test added in `channels_repo_ssh_status_workspace_test.go` (not `channels_test.go`), matching the task's exact assertions (non-empty hostPlatform, honest zero-values, registration check). `go build`/`go vet`/`go test` green in isolation. Not wired into production `RegisterRealChannels` — see TASK-151's status note (the wiring line for this one specifically needs no new client, just `registerStatusChannels(r)` inside `registerRepoSshStatusWorkspaceChannels`, which is already done in this file).
