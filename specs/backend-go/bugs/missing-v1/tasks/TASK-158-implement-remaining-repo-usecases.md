...
**Status:** `[needs merge with git.*/files.* group]` — `BaseRefDefault`/`SearchRefs`/`CheckHooks`/`ReadIssueCommand`/`WriteIssueCommand`/`ScanSetupScriptImports` added to `GitExecutor`, the 6 usecases added, `localgit.Executor` real implementations added (git symbolic-ref/for-each-ref, .git/hooks dir listing, .orca/issue-command.json read/write, best-effort setup-script import scan), `grpcclient.RelayExecutor` relay implementations added. `go build`/`go vet`/`go test` green in this worktree. Same merge-risk note as TASK-156/157.
