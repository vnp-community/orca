...
all touch `ports.go`.
**Status:** `[blocked]` Skipped entirely per the parent scope's explicit instruction — this task's own "Contract correction" section flags 4 of its 9 methods (`checkout`, `localBranches`, `fastForward`, `conflictOperation`) as genuine shape redesigns needing real agent source (`git-handler-push-target.ts`, `git-handler-status-ops.ts`, etc.) open, which this pass did not have time to do properly. Rather than build only the 5 non-blocked methods (`abortRebase`, `abortMerge`, `discard`, `bulkDiscard`, `rebaseFromBase`) in isolation and leave the group half-done, this whole group was left untouched — no proto/usecase/adapter changes for any of the 9 methods. TASK-212's Group-A channel wiring is correspondingly also not present (only its Group-B staging channels are wired, in `channels_git.go`).
