...
**Status:** `[x]` DONE (verified — `go build`/`go vet` clean for `services/api-gateway/internal/adapter/wscompat/...`; `go test ./services/api-gateway/internal/adapter/wscompat/... -run Browser -v` passes, including `TestBrowserProfileList_Success`/`TestBrowserProfileCreate_Success`/`TestBrowserProfileDelete_Success`/`TestBrowserProfileRelayOps_*`. Per orchestration corrections: no `rpcTimeout` (dropped — `handler.go`'s `invokeTimeout` already wraps dispatch); the relayed profile ops use `resolved.GetConnectionId()`, not `resolved.GetDevServer().GetId()`, for `RelayRequest.ConnectionId`; `registerBrowserProfileChannels` is defined but NOT wired into `channels.go`'s `RegisterRealChannels` — that's a separate final-integration pass.)
