...
**Status:** `[partial]` DONE — usecase tests (Step 1-3: `fakeProvider` extended with all 7 new `ScmProvider` methods + `merge_pull_request_test.go` and one test file per remaining TASK-073/074 usecase; `fakeGitHubProjects` + tests for all 16 TASK-078 usecases including `TestUpdateProjectItemField_RejectsNonGitHubProvider`) and adapter tests (Step 4: `projects_test.go` — GraphQL response parsing, field-kind-to-GraphQL-input mapping table test, work-item-details-by-slug slug parsing, `BranchExists`) are complete and green. wscompat channel tests (Step 5-6) are implemented in `channels_scm_test.go` (not `channels_github_test.go` — same file-location deviation as TASK-080/081) covering a representative 10 of the 26 `github.*`/`github.project.*` channels plus the two explicitly-named required tests (`TestGitHubPRForBranchChannel_ReturnsNilWhenNotFound`, `TestGitHubUpdateIssueChannel_OmitsUnsetOptionalFields`) and a `TestGitLabRateLimitChannelMatchesRESTContract`-style contract test — NOT one test per every remaining channel (16 untested channels follow the identical decode-call-return pattern already proven by the tested ones). All present tests pass: `go test ./internal/usecase/... ./internal/adapter/github/...` and `go test ./internal/adapter/wscompat/...` both green; `buf breaking` clean.

**Integration pass still needs** (exact one-line wiring, not done here per the "don't touch channels.go" instruction):
- `services/api-gateway/internal/adapter/wscompat/channels.go`: add `scmClient scmintegrationv1.ScmIntegrationServiceClient` param to `RegisterRealChannels`, import `scmintegrationv1 "github.com/stablyai/orca-go/proto/gen/go/orca/scmintegration/v1"`, and call `registerSCMChannels(r, scmClient)` inside it.
- `services/api-gateway/cmd/server/main.go`: pass the already-dialed `scmClient` through: `wscompat.RegisterRealChannels(wsCompatRegistry, annotationClient, taskClient, gitClient, automationClient, infraFleetClient, scmClient)` (adjust the exact param list to match whatever other groups' parallel channel additions land as).
