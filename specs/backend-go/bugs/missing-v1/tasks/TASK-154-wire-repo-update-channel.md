...
**Status:** `[partial]` `repo.update` implemented inside `registerRepoChannels` in the new `channels_repo_ssh_status_workspace.go` file (not `channels.go`). Builds/tests green in isolation; not wired into production `RegisterRealChannels` — see TASK-151's status note for the exact integration line.
