...
**Depends on:** TASK-034
**Status:** `[blocked]` Product decision required — SOL-006 requires sign-off on whether backend-go should support live remote browser panes (`browser.*`) and, if so, whether `agent/` grows a CDP/Playwright driver on its own roadmap or waits, before any agent-side implementation work can start; TASK-031–TASK-035's backend-go plumbing (this batch) is shipped-but-inert (dead code path until the agent side exists) and safe to merge regardless of this decision's outcome.
