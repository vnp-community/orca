...
**Status:** `[x]` DONE (verified) — `ListSshTargets`/`GetSshState`/`EstablishConnection` usecases added, `Server`/`New(...)` extended (this service has no other parallel group touching these files, unlike git-gateway-service), `cmd/server/main.go` wires all 4 new usecases. Used `apperrors.KindFailedPrecondition` instead of the doc's placeholder `KindUnavailable` (that Kind does not exist in `common/apperrors`'s closed enum). `go build`/`go vet`/`go test` green.
