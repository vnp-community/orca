...
**Status:** `[x]` DONE for the scope this pass closed: all 6 of TASK-209/210's previously-BLOCKED methods (`commitCompare`, `branchCompare`, `commitDiff`, `branchDiff`, `submoduleStatus`, `fetch`) plus `upstreamStatus`'s structured-`pushTarget` change now have real tests across all 3 adapter layers. `localgit.Executor`: 13 new tests against a real `git` binary — `CommitCompare` (diffs-against-parent, root-commit-empty-parent, invalid-commit-id), `BranchCompare` (compares-current-HEAD-against-baseRef, invalid-base-ref, rejects-baseRef-starting-with-dash), `CommitDiff` (returns-file-content-against-parent, root-commit-empty-original, rejects-non-full-object-id), `BranchDiff` (returns-file-content-against-merge-base), `SubmoduleStatus` (real `git submodule add` fixture), `Fetch` (real bare/clone-remote fixture asserting the remote-tracking ref actually advances, both with an explicit `pushTarget.RemoteName` and with `nil`). `RelayExecutor`: 12 new/updated wire-shape tests in `grpcclient_test.go` covering every new method's real param/response shape (`commitId`/`baseRef`/`commitOid`+`parentOid`?+`filePath`+`oldPath`?/`submodulePath`+`area`?/structured `pushTarget`), plus `UpstreamStatus`'s now-structured-pushTarget param and nil-omits-field case. `adapter/grpc/server_test.go`: 8 new translation tests (`UpstreamStatus` with structured pushTarget, `CommitCompare`, `BranchCompare`, `CommitDiff` incl. missing-filePath validation, `BranchDiff`, `SubmoduleStatus` incl. missing-submodulePath validation, `Fetch`). `usecase` dispatch-routing tests for all 6 methods plus `UpstreamStatus`'s pushTarget-forwarding also added in `dispatch_test.go` (see TASK-214's own scope). `go test ./internal/adapter/...` and `go test ./internal/usecase/...` both pass. Everything TASK-215's original pass already covered (Stage/Unstage/History/CheckIgnored/ForkSync/RemoteCommitURL/RemoteFileURL/files.*/localfs) is unchanged.
