...
**Status:** `[x]` DONE (verified, migration NOT applied to a live DB) — `Connection.Status`/`LastActivityAt` added to domain, migration `0004_connection_status.up.sql`/`.down.sql` added, `SshTargetRepository.List`/`ConnectionRepository.GetActiveByDevServer`/`DevServerRepository.FindBySshTarget` added to ports.go and implemented in `postgres/repository.go` (also updated `CreateConnection`'s INSERT to persist the new columns). `go build`/`go vet`/`go test` green. Migration NOT run against a live Postgres — no DB running in this worktree per task instructions; SQL reviewed by hand against migrations/0002's existing style.
