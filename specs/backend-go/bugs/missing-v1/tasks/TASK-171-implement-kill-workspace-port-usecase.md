...
**Status:** `[x]` DONE (verified) — `KillWorkspacePort` usecase added (same resolve-then-dispatch shape as `ScanWorkspacePorts`), wired into `Server`/`New(...)`/`cmd/server/main.go`. `go build`/`go vet`/`go test` green.
