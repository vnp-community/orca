...
**Status:** `[x]` DONE (verified) — all 8 usecase test files added in `git-gateway-service/internal/usecase` (dispatch-to-local-vs-relay coverage, extending the existing `fakeGitExecutor`/`fakeConnectionResolver` rather than duplicating fakes), plus `fakeDevServerReachability` for Clone/InitRepo. `channels_repo_ssh_status_workspace_test.go` (new file, not `channels_test.go`) covers all 8 git-gateway-owned `repo.*` channels plus a registration-coverage regression guard for all 13 `repo.*` channels. `go test` green for both packages.
