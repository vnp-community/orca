...
**Status:** `[x]` DONE — the prior `[partial]` downgrade's gap (TASK-186's correction note: these tests proved the ack+push registration mechanism and per-connection isolation work, but only against `terminal.output`/`terminal.exited` JSON push events, not `terminal.multiplex`'s real binary `TerminalStreamOpcode` framing) is now closed. `channels_terminal_multiplex_test.go` (new) adds real coverage for the binary protocol: `TestTerminalMultiplexChannel_SubscribeOpensAttachPtyAndForwardsOutput`/`_ExitedSendsErrorFrame`/`_InputForwardsRawBytesToPtyStream`/`_ResizeCallsResizeRPC`/`_UnsubscribeStopsForwardingFurtherInput`/`_ConnectionCloseTearsDownEverySlot` drive `registerTerminalMultiplexChannel`'s `BinaryStreamChannelHandler` directly against a `fakeBinaryStreamIO` test double (reusing `channels_terminal_test.go`'s `fakeTerminalInfraFleetClient`/`fakePtyStream`), and `TestTerminalMultiplexChannel_EndToEndSubscribeOutputAndInput` drives the REAL `Handler.ServeHTTP` path with actual binary `websocket.Conn` frames on the wire — the direct counterpart of `TestTerminalCreateChannel_EndToEndPushInterleavesWithConcurrentSend` for the binary protocol, and the test that caught a real bug during development (see TASK-186's "Correction resolved" note). `terminal_stream_frame_test.go` (new) round-trips the frame codec and locks in the exact TS byte layout via a hand-computed fixed-byte-sequence assertion. All of the below (pre-existing, unchanged) still holds: `channels_terminal_test.go` proves the push-bridge wiring genuinely works, not just that it builds: `TestTerminalCreateChannel_ForwardsOutputAndExitPushEvents` drives a fake `AttachPty` stream through `terminal.create`'s returned `<-chan PushEvent` and asserts a real `terminal.output` event for injected `PtyServerFrame_Out` data, a `terminal.exited` event carrying the real exit code for `PtyServerFrame_Exited`, and that the channel closes afterward; `TestTerminalCreateChannel_StreamErrorClosesEventsWithoutHanging` covers the transport-error close path; `TestTerminalCreateChannel_EndToEndPushInterleavesWithConcurrentSend` drives the REAL `Handler.ServeHTTP`/`pipePush`/`handleInvoke` path over an actual `websocket.Conn` pair (same harness `channels_push_test.go`'s `notifications.subscribe` integration test uses) and proves a concurrent `terminal.send` invoke and a `terminal.output`/`terminal.exited` push both arrive intact on the same connection, sharing `writeMu` without corruption. `TestTerminalStreamRegistry_IsSharedAcrossAllConnections` (the old characterization test asserting the FORMER buggy shared-registry, last-writer-wins behavior) is replaced by `TestTerminalStreamRegistry_IsolatesConnectionsWithTheSamePtyID`, which now asserts the FIXED, correct behavior: two simulated connections (two separate `terminalStreamsContext` values, mirroring `Handler.ServeHTTP` constructing one per real WS connection) that create/attach the SAME `ptyId` never cross-talk — `terminal.send`/`terminal.close` on one connection's context only ever reaches that connection's own `AttachPty` stream. `TestTerminalSendChannel_ConcurrentSendsAreSerializedBySendMu` (real `sendMu` race coverage across 32 concurrent `terminal.send` invokes) is retained unchanged from the prior pass. Unknown-`ptyId` send and close-then-send error cases, plus every other terminal.* channel's existing coverage, are retained and updated to the new per-connection-context calling convention. All new and pre-existing tests pass under `-race`.
>
> `methods_test.go` WAS touched this pass (TASK-183 follow-up, same pass as this one): added `TestClientSendSignal_SendsExpectedParams`/`TestClientSendSignal_RejectsUnknownSignal_WithoutACall` for the new `SendSignal` port method, and extended `TestClientAgentStatusAndInspectProcess_FromListProcesses`'s fixture with a `pid` field to assert `InspectProcess.Pid` is now real. A new `internal/usecase/stop_terminal_process_test.go` was also added (`TestStopTerminalProcess_RequiresTenantContext`/`_UnknownPty_ReturnsNotFoundError`/`_SendsRealSIGINT_NotWritePtyCtrlC`/`_AgentFailurePropagates`) — the regression guard proving `StopTerminalProcess` calls `SendSignal(ptyId, "SIGINT")`, not `WritePty([]byte{0x03})`. `spawn_terminal_session_test.go`/`attach_pty_test.go`/`wait_terminal_session_test.go` remain NOT touched — still out of this pass's scope (Part 1 was limited to the `SendSignal`/`InspectProcess.Pid` gap TASK-183 flagged, Part 2 to `api-gateway`'s `wscompat` package). Carry forward the prior pass's spot-check note: these existing tests already cover `ConnectionStreamLimiter` rejection, ctx-cancellation stream teardown, and exit/timeout/ignored-output-event `Wait` cases; the orphaned-PTY `KillPty`-rollback-on-`TerminalSessionRepository.Create`-failure case is still NOT covered (a real implementation gap in `spawn_terminal_session.go`, not just a missing test) and remains open — this is a DIFFERENT, unrelated gap from TASK-186's protocol-mismatch correction this pass closes. `go build ./... && go vet ./... && go test ./... -race` is clean for BOTH `api-gateway` and `infra-fleet-service` in this pass (verify commands below).
