...
**Status:** `[partial]` Test file written (`//go:build e2e`) and compiles clean (`go build`/`go vet -tags e2e`); ran it locally against a real testcontainers-go Postgres + `localhost:9091` and confirmed the full harness (container start, migrations, automation create, `RunNow.Execute`) works end-to-end, failing only at the final gRPC dial with `Unavailable: connection error ... EOF` because no live workflow-service is reachable in this sandbox — expected, not a code defect. **Not actually verified against a real workflow-service** (no docker-compose stack available here), so SOL-033's core claim remains unconfirmed pending a run with `workflow-service` up. Two sketch fields didn't exist in real code and were adapted: `domain.RunStatusSkippedUnavailable` (domain's `RunStatus` has no such constant at all — asserted `final.Status.Terminal()` instead, which structurally can't be skipped_unavailable) and `domain.Automation.LastRunAt` (doesn't exist; assertion dropped, noted as a gap — RunNow's only durable side effect is the `AutomationRun` row, not anything back on `Automation`). Polls via `AutomationRunRepository.FindByRequestID` (no `Get`-by-run-id method exists; used the existing port method instead of adding one).
