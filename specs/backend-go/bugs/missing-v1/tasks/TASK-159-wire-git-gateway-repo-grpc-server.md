...
**Status:** `[needs merge with git.*/files.* group]` — `Server`/`New(...)` extended with the 8 new usecase fields/params, 8 new RPC methods added (translate-only, matching `GetStatus`'s shape), `cmd/server/main.go` wires all 14 usecases into `grpc.New(...)`. `go build`/`go vet`/`go test` green in this worktree. This is exactly the file (`internal/adapter/grpc/server.go`) and composition root (`cmd/server/main.go`) the parallel git.*/files.* group is also extending — needs a manual merge, same note as TASK-156/157/158.
