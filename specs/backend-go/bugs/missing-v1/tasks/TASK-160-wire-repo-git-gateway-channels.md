...
**Status:** `[partial]` All 8 handlers (repo.clone/baseRefDefault/searchRefs/create/hooksCheck/issueCommandRead/issueCommandWrite/setupScriptImports) implemented inside `registerRepoChannels` in the new `channels_repo_ssh_status_workspace.go` file (not `channels.go`, per instructions). `repo.clone` uses the 60s override deadline as specified. Builds/tests green in isolation; not wired into production `RegisterRealChannels` — see TASK-151's status note.
