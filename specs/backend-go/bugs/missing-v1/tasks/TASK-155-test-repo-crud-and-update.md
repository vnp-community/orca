...
**Status:** `[x]` DONE (verified) — `update_repo_test.go` added in `project-service/internal/usecase` (field-mask semantics + authz cases), `channels_repo_ssh_status_workspace_test.go` added in `api-gateway/internal/adapter/wscompat` covering repo.add/list/reorder/rm/update against fake `ProjectServiceClient`/`GitGatewayServiceClient`. `go test` green for both. No Postgres integration test run (no live DB in this worktree).
