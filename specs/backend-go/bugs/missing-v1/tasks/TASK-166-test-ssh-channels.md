...
**Status:** `[x]` DONE (verified) — `list_ssh_targets_test.go`/`get_ssh_state_test.go`/`establish_connection_test.go` added in `infra-fleet-service/internal/usecase`, extending existing fakes (`fakeSshTargetRepository`/`fakeDevServerRepository`/`fakeConnectionRepository`/`fakeDevServerAgentClient`) rather than duplicating. `channels_repo_ssh_status_workspace_test.go` (new file) covers all 4 ssh.* channels including the "getUserAccount derives from ListSshTargets, not a dedicated RPC" assertion. `go test` green for both. Contract test (REST create + WS list round-trip) skipped — no live-DB dual-transport test harness in this worktree, noted as a follow-up.
