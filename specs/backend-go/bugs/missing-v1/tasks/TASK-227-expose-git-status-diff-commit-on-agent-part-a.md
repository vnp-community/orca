...
**Depends on:** none
**Status:** `[x]` DONE — **P0 blocker resolved.** All 20 methods wired. Extracted 16 `GitHandler` private methods into 3 new ops modules (`git-handler-staging-ops.ts`, `git-handler-branch-ops.ts`, `git-handler-remote-sync-ops.ts`); 20 re-export handlers split across 2 new files (`agent-git-handler-local-ops.ts`, `agent-git-handler-remote-ops.ts`) to stay under the 300-line oxlint budget; 20 new dispatch cases in `agent-rpc-dispatch.ts`. 12/12 new tests pass (real `git` subprocesses); existing 281-test `git-handler` suite unaffected; full `npm test` 3744 passed (3 pre-existing unrelated failures, confirmed identical to baseline). Worktree `agent-a9f26c7aecfec4d91`, branch `worktree-agent-a9f26c7aecfec4d91`, **committed** as `a4ca579e7`. Pending merge to main.
