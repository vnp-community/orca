...
**Depends on:** none
**Status:** `[needs merge with git.*/files.* group]` — 8 new RPCs (`Clone`/`BaseRefDefault`/`SearchRefs`/`InitRepo`/`CheckHooks`/`ReadIssueCommand`/`WriteIssueCommand`/`ScanSetupScriptImports`) + messages added to `gitgateway.proto`, `google/protobuf/empty.proto` import added, stubs regenerated, `go build ./proto/...` clean IN THIS WORKTREE. Another group is concurrently extending `git-gateway-service` for git.*/files.* work against a copy of this file that does not yet have these changes — this proto file, and the `internal/adapter/grpc/server.go`/`cmd/server/main.go` files this bucket also touches, will need a manual merge/rebase against that group's changes before landing. Flagging explicitly per task instructions even though this worktree's own build/tests are green.
