...
**Status:** `[partial]` The 4 ✅ shippable-now methods (`history`, `checkIgnored`, `forkSync`, `upstreamStatus`) are fully implemented per this task's own Contract correction section shapes (cursor dropped/baseRef rename for history; ignored-subset-only response for checkIgnored; required expectedUpstream for forkSync; optional pushTarget placeholder for upstreamStatus) — proto, domain types (`CommitRef`/`ForkSyncStatus`/`UpstreamStatus`), usecases, `localgit.Executor`, `RelayExecutor`, gRPC adapter, and `main.go` wiring all done; `go build`/`go vet`/`go test` clean. Test coverage verified/closed this pass across all 4 layers: `localgit.executor_test.go` (real-git-binary), `dispatch_test.go` (usecase routing), `grpcclient_test.go` (`RelayExecutor` wire-shape, incl. the previously-missing `UpstreamStatus` param/response test added now), and `grpc/server_test.go` (translation). The 5 ❌ BLOCKED methods (`commitCompare`, `branchCompare`, `commitDiff`, `branchDiff`, `submoduleStatus`) are deliberately NOT implemented — each needs a genuine shape redesign this task's own correction pass explicitly says not to fabricate without the real agent source open. `upstreamStatus` is code-complete but its relay path still needs TASK-227 for reachability (out of this task's scope).
