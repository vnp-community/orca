...
**Status:** `[x]` DONE — `channels_git_test.go`'s `fakeGitGatewayClient` (mirroring `channels_test.go`'s `fakeInfraFleetClient` embed-nil-interface pattern) extended to cover every one of the 18 `git.*` channels actually registered by this pass (`registerGitChannels`'s `git.status`/`git.diff` plus `registerGitDeepChannels`'s 16): `git.diff` (filePath threaded through), `git.commit`, `git.push`, `git.pull`, `git.generateCommitMessage`, `git.stage`/`git.bulkStage` (shared-handler regression), `git.unstage`/`git.bulkUnstage` (shared-handler regression), `git.history` (corrected baseRef/no-cursor shape), `git.checkIgnored` (unwrapped ignored-subset), `git.forkSync` (expectedUpstream param), `git.upstreamStatus` (pushTarget param), `git.remoteCommitUrl`, `git.remoteFileUrl`, `git.generatePullRequestFields`, and `git.discoverCommitMessageModels` (Identity-sourced tenant/user, never client args). The 9 channels TASK-207/209/210 left BLOCKED (`git.checkout`/`git.localBranches`/`git.fastForward`/`git.rebaseFromBase`/`git.abortRebase`/`git.abortMerge`/`git.conflictOperation`/`git.discard`/`git.bulkDiscard`) and the 5 history/compare + fetch BLOCKED methods (`git.commitCompare`/`git.branchCompare`/`git.commitDiff`/`git.branchDiff`/`git.submoduleStatus`/`git.fetch`) are correctly not tested — they're not registered at all (fall through to `notImplementedHandler`), out of this pass's scope. `go build`/`go vet`/`go test ./internal/adapter/wscompat/... -run TestGit` clean (14 `git.*` tests, all passing).
