...
**Status:** `[x]` DONE. `remoteCommitUrl`/`remoteFileUrl` unchanged (already implemented). `fetch` is now implemented for real: TASK-227 (agent reachability) landed and TASK-207 built `domain.PushTargetInput`/proto `PushTargetInput` for real, resolving both blockers this task's own Contract correction section flagged. `Fetch` matches the confirmed real contract exactly (`specs/agent/api/agent-rpc-catalog-git-fs.md:155`, `agent/src/relay/git-handler-push-target.ts`): `worktreePath, pushTarget?` → `git fetch --prune [remote]` (always prunes, no separate prune bool — the original `remote string, prune bool` design is gone). `localgit.Executor.Fetch`, `RelayExecutor.Fetch` (reusing `FastForward`'s existing `pushTargetParam` helper), gRPC adapter, and `main.go` wiring all done; `go build`/`go vet`/`go test` clean. Test coverage added this pass: `localgit.executor_test.go` (real bare/clone-remote fixture asserting the remote-tracking ref actually advances after fetch, both with an explicit `pushTarget.RemoteName` and with `nil`), `grpcclient_test.go` (`RelayExecutor.Fetch` wire-shape — `pushTarget` object shape and nil-omits-field), `grpc/server_test.go` (`Fetch` translation).
