...
**Status:** `[partial]` Group B (staging) only: `git.stage`/`git.bulkStage`/`git.unstage`/`git.bulkUnstage` registered in `channels_git.go`'s `registerGitDeepChannels`, sharing one handler each per this task's own design. Group A (`git.checkout`/`git.localBranches`/`git.fastForward`/`git.rebaseFromBase`/`git.abortRebase`/`git.abortMerge`/`git.conflictOperation`/`git.discard`/`git.bulkDiscard`) NOT wired — TASK-207 (the RPCs these channels would call) was skipped entirely for this pass, see TASK-207's own status note. `go build`/`go vet`/`go test` clean.
