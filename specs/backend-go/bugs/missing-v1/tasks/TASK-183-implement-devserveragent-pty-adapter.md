...
**Depends on:** TASK-181
**Status:** `[x]` DONE — PTY adapter implemented and tested (32/32 passing in `internal/adapter/devserveragent`; 4/4 in `internal/usecase`'s new `stop_terminal_process_test.go`). This pass closed the two deviations the previous `[partial]` note flagged, `agent/` changes now in scope per this pass's instructions:
- **StopTerminalProcess** now calls the real `pty.sendSignal` RPC (confirmed real: `agent/src/relay/agent-rpc-dispatch.ts`'s `case 'pty.sendSignal'` → `pty-daemon-client.ts`'s `handlePtySendSignal` → `pty-agent-bridge.ts`'s real implementation, `Params: {id, signal}`, `ALLOWED_SIGNALS = {SIGTERM,SIGKILL,SIGINT,SIGHUP,SIGTSTP}`) with `signal=SIGINT`, via a new `DevServerAgentClient.SendSignal` port method — replacing the former `WritePty([]byte{0x03})` Ctrl-C-byte workaround entirely. See `usecase/stop_terminal_process.go` and `devserveragent/methods.go`'s `SendSignal`.
- **InspectProcess.Pid** is now real, not always 0: `agent/src/relay/pty-agent-bridge.ts`'s `handlePtyListProcesses` was extended (this pass) to include `pid: entry.pty.pid` in its `{id,cwd,title}` result shape, and `devserveragent/methods.go`'s `ptyProcessSummary`/`InspectProcess` now decode and surface it.
- **AgentStatus.ReadyForInput remains a heuristic** (`== AgentRunning`) — confirmed still genuinely unclosable without a NEW agent-side RPC: `pty.listProcesses` (the only real per-pty enumeration primitive) has no busy/idle signal at all, checked again against the real `pty-agent-bridge.ts` source in this pass. This is an honestly-documented architectural gap (the agent doesn't expose the needed signal anywhere), not leftover adapter work — see `methods.go`'s `AgentStatus` doc comment.
