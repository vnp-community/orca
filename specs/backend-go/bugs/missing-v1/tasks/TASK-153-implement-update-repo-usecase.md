...
**Status:** `[x]` DONE (verified) — `UpdateRepo` usecase added (matching `AddRepo`/`RemoveRepo`'s real port shape: `repo RepoRepository, membership MembershipRepository, opa OPAClient`, owner-only authz), `RepoRepository.Update` added to the port + Postgres adapter, wired into `grpc.Server`/`Deps`/`main.go`. `go build`/`go vet` clean, `go test ./services/project-service/...` green.
