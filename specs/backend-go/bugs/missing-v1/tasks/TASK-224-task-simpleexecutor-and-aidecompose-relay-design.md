...
**Status:** `[partial]` Real `SimpleExecutor`, `ProjectExecutionResolver`, `AICompleter`, `AIProviderContextResolver`, `AIDecompose`/`AIApply` usecases, proto RPCs, gRPC server wiring, and `main.go` dialing of infra-fleet-service + ai-provider-service are all implemented, unit-tested, and passing (`go build`/`go vet` clean; see TASK-226). `ComplexExecutor` stays a stub as specified — out of scope. The one piece NOT fully resolved: `SimpleExecutor`'s `agent.exec` relay method name/params/result shape is the task's own best-effort guess, kept as specified rather than switched — `specs/agent/api/agent-rpc-catalog-runtime.md` (confirmed present) documents `agent.exec`'s REAL contract as a `binary/args/cwd`-based "run this literal binary" primitive with **no live backend caller as of 2026-08-16**, superseded by `agent.execPrompt` (`prompt`+`worktreePath`-based) for exactly this "dispatch an AI-driven task" use case — but switching to `agent.execPrompt` needs a real design decision (worktreePath resolution, prompt construction, model/account selection) not answered by this port's existing signature or any existing task-service code, so it isn't guessed at here. `ai.complete` (`AICompleter`/`AIDecompose`'s relay call), by contrast, IS confirmed correct against that same spec — identical to the task's original guess. Flagged in `simple_executor.go`'s doc comment; see final report for detail. AIApply's known non-transactional gap (no `WithTx`/`UnitOfWork` precedent anywhere in this repo; the create-subtask+add-edge loop is not wrapped in a single DB transaction) is likewise implemented-as-specified-and-flagged, not silently ignored — see `ai_apply.go`'s doc comment.
