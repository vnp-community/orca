...
**Status:** `[x]` DONE. All 14 channels this task covers are now wired in `channels_git.go`'s `registerGitDeepChannels`, back with real RPCs: the 8 previously-wired channels (`git.history`/`git.checkIgnored`/`git.forkSync`/`git.upstreamStatus`/`git.remoteCommitUrl`/`git.remoteFileUrl`/`git.generatePullRequestFields`/`git.discoverCommitMessageModels`) are unchanged except `git.upstreamStatus`, which now decodes the real structured `pushTarget` (`{remoteName,branchName,remoteUrl?,remoteCreated?}`, same shape `git.fastForward` already decoded) instead of a placeholder bare string. The 6 previously-NOT-wired channels are now wired for real against TASK-209/210's resolved shapes: `git.commitCompare`(`worktreeId,commitId`)/`git.branchCompare`(`worktreeId,baseRef`) — one fixed ref, not two arbitrary refs; `git.commitDiff`/`git.branchDiff` — required `filePath` (+ optional `parentOid`/`oldPath`), returning the new `FileDiffResponse` shape, not `GetDiffResponse`; `git.submoduleStatus`(`worktreeId,submodulePath,area?`) — per-single-submodule, no "list all" aggregation needed (see TASK-209's resolution of SOL-032 §0 open question #3); `git.fetch`(`worktreeId,pushTarget?`) — same structured-`pushTarget` decode as `upstreamStatus`/`fastForward`. All 14 of this task's channels are therefore fully wired; `cancelGenerateCommitMessage`/`cancelGeneratePullRequestFields` remain the only 2 unregistered names in the frontend's `git.*` catalog, per this task's own Context section (out of scope — no server-side job to cancel). `go build`/`go vet`/`go test` clean, including 12 new/updated wscompat channel tests (`channels_git_test.go`) covering every new/changed channel's arg-decode and pushTarget-omission-when-nil behavior.
