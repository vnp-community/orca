...
| What it models | One logged-in server user's **config cascade**: Company → Department → Team → User, merged into agent/editor/shell/security settings (`docs/guides/profile/user-profile-team-department-rbac.md`) | Multiple **local app identities on one machine** (like Chrome/Firefox profiles), each with its own repos/sessions/settings, one of which is "active" in the current process |
