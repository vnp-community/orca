...
## v5.0 — Architecture Updates

### Process Model (v5.0)

```
┌─────────────────────────────────────────────────────────────────────────────────┐
│                         ORCA WEB SERVER (Node.js)                               │
│                         src/server/index.ts                                     │
│                                                                                 │
│  Server Bootstrap Order (server-bootstrap.ts):                                  │
│  1. setPlatform(NodeAdapter)          ← ADR-001                                │
│  2. initDatabase() + runMigrations()  ← ADR-002 (migrations 0001–0010)         │
│  3. AuthManager + SessionManager      ← ADR-003                                │
│  4. DevServerManager                  ← ADR-004                                │
│  5. AgentWebSocketServer.attach()     ← ADR-005                                │
│  6. initWebCredentialStore()          ← ADR-006                                │
│  7. ProfileService + ProjectService   ← ADR-007 [v5.0]                        │
│  8. AIProviderService                 ← ADR-008 [v5.0]                        │
│  9. WorkflowOrchestrator              ← ADR-009 [v5.0]                        │
│  10. TaskService                      ← ADR-010 [v5.0]                        │
│  11. RelayConnectionPool init         ← ADR-011 [v5.0]                        │
│  12. HTTP :6769 + WS :6768 listen     ← existing                              │
│                                                                                 │
│  [NEW v5.0] Services:                                                           │
│  ├── ProfileService (OrcaProfile 3-layer merge)                                │
│  ├── ProjectService (project CRUD + membership)                                │
│  ├── AIProviderService (7 providers, relay-only credentials)                   │
│  ├── WorkflowOrchestrator (DAG, wave execution, resumable)                     │
│  ├── TaskService (DAG tasks, grants, AI planning)                              │
│  └── RelayConnectionPool (reuse SSH relay per dev server)                      │
└─────────────────────────────────────────────────────────────────────────────────┘
        │ Per-user fork (ADR-003)
        ▼
┌──────────────────────────────┐
│  USER PROCESS (per userId)   │
...
└──────────────────────────────┘
        │ relay.call() via RelayConnectionPool
        ▼
┌──────────────────────────────────────────────────────┐
│  DEV SERVER (remote)                                 │
│                                                      │
│  Relay binary (auto-deployed via SFTP):              │
│  ├── fs.readDir / fs.read / fs.grep                 │
│  ├── agent.exec (PTY spawn)                          │
│  ├── git.exec (generic git, whitelist validated)     │  ← NEW TDD-20
│  ├── ai.provider.writeCredential                     │  ← NEW TDD-16
│  └── ai.provider.readCredential (for agent spawn)    │  ← NEW TDD-16
│                                                      │
│  AI credential files:                                │
...
### v5.0 ServerBootstrapResult (thêm fields mới)
...
### RPC Method Namespaces (v5.0 additions)

| Namespace | TDD | Feature |
|-----------|-----|---------|
| `profile.*` | TDD-14 | F33 Profile Hierarchy |
| `project.*` | TDD-15 | F34 Project Binding |
| `aiProvider.*` | TDD-16 | F35 AI Provider |
| `workflow.*` | TDD-17 | F36 Workflow Orchestration |
| `task.*` | TDD-18 | F37 Task Graph |
| `workspace.*` | TDD-19 | F38 Project Workspace |
| `git.*` (remote ext.) | TDD-20 | F39 Remote Git UI |
