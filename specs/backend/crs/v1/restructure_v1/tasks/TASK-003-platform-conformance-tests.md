# TASK-003: Tạo conformance test helpers
...
Tạo file `src/platform/__tests__/interface-conformance.ts` chứa các helper functions để test conformance của bất kỳ adapter nào. Đây là "test suite" dùng chung cho cả NodeAdapter và ElectronAdapter.
...
import type { IWindow, IWindowManager } from '../window-interface'
import type {
  IIpcBridge,
  IpcEvent,
  IpcListener
...
export function runIIpcBridgeConformanceTests(
  ...
  describe('IIpcBridge conformance', () => {
    ...
    it('handle() + invoke() roundtrip', async () => {
      ipc.handle('test:double', async (_e, n: number) => n * 2)
...
# File phải compile clean
npx tsc --noEmit 2>&1 | grep "interface-conformance" | head -10

# File phải có thể import trong test files
node -e "require('./src/platform/__tests__/interface-conformance.ts')" 2>&1 | head -5
```
...
- [x] `src/platform/__tests__/interface-conformance.ts` tồn tại
- [x] Export: `runIAppConformanceTests`, `runIWindowConformanceTests`, `runIIpcBridgeConformanceTests`
- [x] Không có syntax errors (TypeScript compile clean)
- [x] Sẵn sàng để import trong TASK-004, TASK-005, TASK-006
