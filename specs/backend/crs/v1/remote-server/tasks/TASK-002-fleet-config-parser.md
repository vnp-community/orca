# TASK-002: Tạo `fleet-config-parser.ts`
...
Tạo file mới `src/main/ssh/fleet-config-parser.ts` — parser cho file `orca-fleet.yaml` sử dụng Zod schema validation + js-yaml.

---

## File to create

**`src/main/ssh/fleet-config-parser.ts`** (NEW)
...
/**
 * Parse an orca-fleet.yaml file and validate against schema.
 * @throws ZodError if validation fails
 * @throws Error if file not found or YAML parse error
 */
...
/**
 * Parse fleet config from a YAML string (for testing).
 */
...
export function fleetServerToSshTarget(
  ...
  return {
    ...
    tags: merged.tags,
    repos: merged.repos?.map(r => ({ path: r.path, name: r.name, url: r.url, branch: r.branch })),
    fleetId: server.id,
...
/**
 * Serialize a list of SshTargets back to FleetConfig format.
 */
...
npx tsc --noEmit 2>&1 | grep fleet-config-parser | head -20
```
...
- [x] File `src/main/ssh/fleet-config-parser.ts` tồn tại
- [x] `parseFleetConfig()` export async function
- [x] `fleetServerToSshTarget()` export function
- [x] `sshTargetsToFleetConfig()` export function
- [x] Zod schema validates `version: "1"` + `servers[]`
- [x] TypeScript compile: no errors

**Status: ✅ DONE** — `src/main/ssh/fleet-config-parser.ts` created. Uses `yaml` v2 (project standard) + Zod v4. Exports: `parseFleetConfig`, `parseFleetConfigFromString`, `fleetServerToSshTarget`, `sshTargetsToFleetConfig`, `sshTargetsToFleetConfigYaml`.
