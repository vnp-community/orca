# TASK-DB-008: Tạo `src/main/db/pool.ts` — IConnectionPool interface ✅ DONE

**Source:** SOL-DB-002 §4.1  
**Phase:** 2 | **Effort:** XS (< 20 min) | **Status:** ✅ COMPLETED 2026-07-23  
...
Tạo `src/main/db/pool.ts` định nghĩa `IConnectionPool` interface và `PoolStats` type — shared interface mà cả `SqliteSingleConnectionPool` và `GenericConnectionPool` phải implement.
...
# TypeScript compile
npx tsc --noEmit 2>&1 | grep "db/pool" | head -10
```

Expected: Zero TS errors, file compiles cleanly.

---

## Done criteria

- [x] `src/main/db/pool.ts` tồn tại
- [x] Export `PoolStats`, `PoolConfig`, `DEFAULT_POOL_CONFIG`, `IConnectionPool`
- [x] `IConnectionPool` có 7 methods: acquire, release, withConnection, withTransaction, stats, drain, destroy
- [x] `DEFAULT_POOL_CONFIG` có default values (min=2, max=10, timeout=5000)
- [x] Không có implementation code — chỉ là interfaces và constants
