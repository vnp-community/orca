# TASK-011: ProfileResolver Tests

**Phase:** 2 — Profile Hierarchy  
**Solution ref:** [SOL-V5-001](../solutions/SOL-V5-001-profile-hierarchy.md) §6  
**Prerequisite:** TASK-008 (ProfileResolver)  
**Status:** ✅ DONE — 2026-07-28

---

## File cần tạo: `src/main/profile/__tests__/ProfileResolver.test.ts`
...
// 3. User field overrides dept field overrides company field
...
it('invalidate(userId) clears specific user', async () => {
  ...
  expect((svc.getUserProfile as any).mock.calls.filter((c: any[]) => c[0] === 'u-1')).toHaveLength(2)
  expect((svc.getUserProfile as any).mock.calls.filter((c: any[]) => c[0] === 'u-2')).toHaveLength(1)
})
...
- [x] ≥ 14 tests
- [x] All tests pass
- [x] Tests cover: cache hit/miss, locked section, pathAdditions concat, envVars override, mcp dedup, invalidate
