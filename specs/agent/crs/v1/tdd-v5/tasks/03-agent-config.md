# TASK-03: Create src/relay/agent-config.ts

**Phase:** 1  
**SOL Ref:** SOL-03  
**Estimated time:** 30m  
**Precondition:** TASK-02 hoàn thành (agent-logger.ts tồn tại)  

---

## Tạo file mới: `src/relay/agent-config.ts`
 ...
 * Throws on invalid MODE to fail fast at startup.
 */
...
```bash
pnpm run typecheck:node 2>&1 | grep "agent-config" || echo "No errors"
```
