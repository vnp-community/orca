...
// Why: OrcaRuntimeService's narrower RuntimeStore doesn't type activeRuntimeEnvironmentId
// on getSettings(), so the local RPC path (which only has RuntimeStore) can't satisfy
// RuntimeEnvironmentFocusStore structurally. At runtime this is always the same concrete
// Store instance either way; the guarded cast just restores the narrower ipc-only type.
type FocusClearableStore = Store | (EphemeralVmRecipeStore & Partial<Store>)

function tryClearActiveRuntimeEnvironmentFocus(
  store: FocusClearableStore,
  environmentId: string
): void {
  if (typeof store.getSettings !== 'function' || typeof store.updateSettings !== 'function') {
    return
  }
  clearActiveRuntimeEnvironmentFocusIfMatches(
    store as unknown as Parameters<typeof clearActiveRuntimeEnvironmentFocusIfMatches>[0],
    environmentId
  )
}

export function listEphemeralVmRuntimeRecords(): EphemeralVmRuntimeRecord[] {
...
export async function cleanupEphemeralVmWorkspace(
  ...
  if (result.ok && runtime.runtimeEnvironmentId) {
    try {
      ...
      tryClearActiveRuntimeEnvironmentFocus(store, runtime.runtimeEnvironmentId)
    } catch {
...
export async function suspendEphemeralVmWorkspace(
  store: EphemeralVmRecipeStore,
...
export function getEphemeralVmCleanupCommand(
  store: EphemeralVmRecipeStore,
  args: { runtimeId: string }
): EphemeralVmCleanupCommandResult {
  const userDataPath = app.getPath('userData')
...
export function registerEphemeralVmRuntimeHandlers(store: Store): void {
  ...
  ipcMain.handle(
    ...
    (_event, args: { runtimeId: string; workspaceId: string }): EphemeralVmRuntimeRecord =>
      attachEphemeralVmWorkspace(args)
  )

  ipcMain.handle(
    'ephemeralVm:cleanup',
    (_event, args: { runtimeId: string }): Promise<EphemeralVmRuntimeRecord> =>
      cleanupEphemeralVmWorkspace(store, args)
  )

  ipcMain.handle(
    'ephemeralVm:suspendWorkspace',
    (_event, args: { workspaceId: string }): Promise<EphemeralVmRuntimeRecord | null> =>
      suspendEphemeralVmWorkspace(store, args)
  )

  ipcMain.handle(
    'ephemeralVm:resumeWorkspace',
    (_event, args: { workspaceId: string }): Promise<EphemeralVmRuntimeRecord | null> =>
      resumeEphemeralVmWorkspace(store, args)
  )

  ipcMain.handle(
    'ephemeralVm:getCleanupCommand',
    (_event, args: { runtimeId: string }): EphemeralVmCleanupCommandResult =>
      getEphemeralVmCleanupCommand(store, args)
  )
}
