...
export async function openInFileManager(pathValue: string): Promise<ShellOpenLocalPathResult> {
...
export async function openWithSystemDefault(pathValue: string): Promise<boolean> {
  const target = await validateLocalPathTarget(pathValue)
  if (!target.ok) {
    return false
  }
...
// Why: RPC (window.api.runtime.call) and legacy ipcMain.handle both need the
// exact same open-URL/open-file-uri/picker/copy logic; extracting these into
// named exports lets the RPC method wrappers call the identical function
// instead of re-implementing the validation and dialog behavior.
export function openUrl(rawUrl: string): Promise<void> | void {
  let parsed: URL
  try {
...
export async function openFileUri(rawUri: string): Promise<void> {
  let parsed: URL
  try {
...
export async function pickDirectory(args: { defaultPath?: string }): Promise<string | null> {
...
export async function pickAttachment(): Promise<string | null> {
  const result = await dialog.showOpenDialog({
    properties: ['openFile']
  })
  if (result.canceled || result.filePaths.length === 0) {
...
export async function copyPickedFile(args: {
  srcPath: string
  destPath: string
}): Promise<void> {
...
export function registerShellHandlers(): void {
  ...
  ipcMain.handle('shell:openUrl', (_event, rawUrl: string) => openUrl(rawUrl))

  ipcMain.handle('shell:openFilePath', async (_event, filePath: string): Promise<boolean> => {
    return openWithSystemDefault(filePath)
  })

  ipcMain.handle('shell:openFileUri', (_event, rawUri: string) => openFileUri(rawUri))
  ...
  ipcMain.handle(
    'shell:pickDirectory',
    (_event, args: { defaultPath?: string }): Promise<string | null> => pickDirectory(args)
  )

  ipcMain.handle('shell:pickAttachment', (): Promise<string | null> => pickAttachment())

  ipcMain.handle('shell:pickImage', (): Promise<string | null> => pickImage())

  ipcMain.handle(
    'shell:pickRepoIconImage',
    (): Promise<{ dataUrl: string; fileName: string } | null> => pickRepoIconImage()
  )

  ipcMain.handle('shell:pickAudio', (): Promise<string | null> => pickAudio())

  ipcMain.handle(
    'shell:copyFile',
    (_event, args: { srcPath: string; destPath: string }): Promise<void> => copyPickedFile(args)
  )
}
