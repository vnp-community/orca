...
vi.mock('../LinkedProjectsManager', () => ({
  LinkedProjectsManager: ({ orcaProjectId, currentUserRole }: { orcaProjectId: string; currentUserRole: string | null }) => (
...
vi.mock('../../ui/tabs', () => ({
  ...
  TabsContent: (p: any) => <div>{p.children}</div>
}))
...
describe('ProjectSettings', () => {
  beforeEach(() => {
    ...
    vi.mocked(callRuntimeRpc).mockImplementation(async (_target, method) => {
      ...
      if (method === 'project.getMembers') {return []}
      return null
    })
  })
  afterEach(cleanup)
  ...
  it('resolves currentUserRole via project.getMembers and passes it to LinkedProjectsManager', async () => {
    vi.mocked(callRuntimeRpc).mockImplementation(async (_target, method) => {
      if (method === 'repo.list') {return { repos: [] }}
      if (method === 'project.getMembers') {return [{ userId: 'u1', role: 'owner' }, { userId: 'u2', role: 'member' }]}
      return null
    })
    render(<ProjectSettings projectId="p1" open={true} onClose={vi.fn()} />)
    ...
    await waitFor(() => {
      expect(screen.getByTestId('linked-projects-manager')).toHaveTextContent('for p1 (role: owner)')
    })
  })
  ...
  it('Repos tab lists project repos via repo.list, scoped to projectId', async () => {
    ...
    render(<ProjectSettings projectId="p1" open={true} onClose={vi.fn()} />)
    await waitFor(() => {
      expect(callRuntimeRpc).toHaveBeenCalledWith(expect.anything(), 'repo.list', { projectId: 'p1' })
    })
    await waitFor(() => {
