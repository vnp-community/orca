...
describe('TaskGrantModal', () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks()
    vi.mocked(useTaskGrants).mockReturnValue({
      grants: [],
      addGrant,
      revoke,
      generateShareLink,
  ...
  it('grants rỗng → hiện message "No grant list available yet"', () => {
    render(<TaskGrantModal taskId="t1" />)
    expect(screen.getByTestId('grant-list-empty')).toBeInTheDocument()
  })

  it('subjectId rỗng → nút "Grant access" disabled', () => {
    render(<TaskGrantModal taskId="t1" />)
    expect(screen.getByTestId('grant-submit')).toBeDisabled()
  })

  it('nhập subjectId, chọn level, bấm "Grant access" → gọi addGrant với đúng giá trị form', () => {
    render(<TaskGrantModal taskId="t1" />)
  ...
  it('grants có dữ liệu → hiện danh sách + nút Revoke wired to revoke', () => {
    vi.mocked(useTaskGrants).mockReturnValue({
