...
// FIX BUG-FE-HLD-001: deviceToken is now stored wrapped (see
// web-runtime-environment-crypto.ts), not plaintext. This helper must write
// the same shape saveStoredWebRuntimeEnvironment() would, using a dynamic
// import so it shares the in-memory wrap key with whatever module instance
// installApi()'s later `await import('./web-preload-api')` resolves to
// post-vi.resetModules() — a static top-level import would bind to a stale
// pre-reset module instance and the wrapped token would fail to unwrap.
async function writeStoredRuntimeEnvironment(storage: Storage): Promise<void> {
  const { wrapDeviceToken } = await import('./web-runtime-environment-crypto')
  storage.setItem(
      ...
      endpoints: [
        {
          ...
          deviceToken: wrapDeviceToken('token'),
