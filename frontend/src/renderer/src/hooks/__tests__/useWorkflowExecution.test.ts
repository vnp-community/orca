...
describe('useWorkflowExecution().cancelExecution() tracing', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    mockStore.executions = [{ id: 'exec-1', templateId: 't1', status: 'running', rootTraceId: 'root-abc' }]
  ...
  it('forwards traceId: span.id into workflow.cancel params, target as first arg', async () => {
    ...
    const startEvent = events.find(e => e.flow === 'ui:workflow.cancel' && e.level === 'start')
    expect(mockRpc).toHaveBeenCalledWith('mock-target', 'workflow.cancel', { executionId: 'exec-1', traceId: startEvent?.id })
  })
